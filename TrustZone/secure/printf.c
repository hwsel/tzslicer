...
int timer_printf(const char *fmt, ...)
{
	...
	char printbuffer[CFG_PBSIZE - 15];
	char printbuffer_with_timestamp[CFG_PBSIZE];

	va_start(args, fmt);

	microsecond=get_timer(0);
	vsprintf(printbuffer, fmt, args);
	i = sprintf(printbuffer_with_timestamp,"[%10u.%03u]%s",microsecond/1000,microsecond%1000,printbuffer);
	va_end(args); 
